...
### app.py — Главно приложение (1528 реда)

Streamlit UI фреймуърк. Управлява layout (45% чат / 55% визуализация), sidebar навигация, session state (17 ключа) и инициализация на всички мениджъри. Проверява конфигурацията при стартиране. Персистентна чат история (последни 10 двойки) с expander за преглед при зареждане на проект.

### src/ai_router.py — Двоен AI маршрутизатор (1068 реда)

Ядрото на AI системата. Маршрутизира заявки между DeepSeek V3 (работник) и Anthropic Claude Sonnet 4.6 (контрольор). Поддържа двупосочен fallback, следи разходите в реално време и кумулативно (across sessions). Stop flag за прекъсване на дълги операции.
...
### src/chat_handler.py — Обработка на чат (1165 реда)

Детектира намерение на потребителя чрез scoring на ключови думи (7 intent типа). Маршрутизира към съответния handler: зареждане на проект, генериране, модификация, експорт, запис на урок, еволюция, общ разговор. Поддържа restore на чат историята от запазени данни.
...
### src/project_manager.py — Управление на проекти (445 реда)

Персистентна история в `config/projects_history.json`. MD5 хеш на пътя = project ID. Последни 5 проекта с бързо възобновяване. Статус-aware welcome съобщения на български. Запазва чат история (последни 10 двойки) за всеки проект.
