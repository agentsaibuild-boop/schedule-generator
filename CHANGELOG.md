...
### Добавено
- Персистентна чат история: последните 10 двойки съобщения се запазват за всеки проект
- При зареждане на проект — info bar с кратко summary + expander за преглед на предишна история
- Auto-save на историята след всяко съобщение и при смяна/изчистване на проект
- Auto-restore на историята при page refresh
- Кумулативно проследяване на AI разходи (across sessions) в `config/cumulative_usage.json`
- Бутон "Спри операцията" за прекъсване на дълги AI процеси (генериране/проверка)

### Поправено
- Критичен бъг: графикът се запазваше като JSON string вместо list[dict] — причиняваше `AttributeError: 'str' object has no attribute 'get'`
- `_ensure_schedule_list()` хелпер парсва JSON/markdown-fenced strings навсякъде (4 места)
