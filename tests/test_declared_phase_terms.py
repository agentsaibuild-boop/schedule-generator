...
def test_phase_tail_is_topped_up_to_the_declared_cap():
    """Тънката настройка е по КРАЯ, не по сбора.

    FAILURE означава, че фазата увисва под тавана и повече не мърда:
    пропорционалното разтягане расте по СБОРА от продължителности, а обхватът
    се държи от критичния път.  Мерено на Илиянци — 650 → 663 → 662 при
    обявени 660, тоест целта се прескача и никога не се улучва.
    """
    from src.segment_scale import _доший_края

    задачи = [{"id": "t1", "duration": 5, "end_day": 10},
              {"id": "t2", "duration": 5, "end_day": 20},
              {"id": "край", "duration": 3, "end_day": 30}]
    пипнати = _доший_края(задачи, 4)
    assert пипнати >= 1
    добавени = sum(int(t["duration"]) for t in задачи) - 13
    assert добавени == 4, "дошиха се други дни, не поисканите"
    # Последното действие държи края — то трябва да е сред пипнатите.
    assert задачи[-1]["duration"] > 3


def test_milestones_are_never_stretched():
    """Милестоунът е ден, не работа — той не се дошива."""
    from src.segment_scale import _доший_края

    задачи = [{"id": "m", "duration": 0, "end_day": 30, "milestone": True},
              {"id": "t", "duration": 4, "end_day": 29}]
    _доший_края(задачи, 3)
    assert задачи[0]["duration"] == 0
    assert задачи[1]["duration"] == 7
...
def test_the_last_day_is_taken_off_the_end_not_the_longest_step():
    """Обхватът го държи КРАЯТ — оттам се маха денят, не от най-дългата стъпка.

    Тръстеник, 25.08.2026: проектирането застояваше на 46 дни при обявени 45.
    Свиването махаше ден от най-дългата стъпка, а тя вървеше УСПОРЕДНО с
    последното действие и краят не помръдваше.  Един ден над обявения максимум
    прави офертата неотговаряща — това не е закръгляне.
    """
    from src.segment_scale import _отрежи_края

    успоредна = {"id": "П", "chain_step": "s0", "duration": 20,
                 "start_day": 1, "end_day": 20}
    последна = {"id": "К", "chain_step": "s1", "duration": 6,
                "start_day": 41, "end_day": 46}
    задачи = [успоредна, последна]

    пипнати = _отрежи_края(задачи, 1)

    assert пипнати == 1
    assert последна["duration"] == 5, "денят не е махнат от последното действие"
    assert успоредна["duration"] == 20, "махнат е ден от успоредна стъпка"
    assert последна["computed_duration"] == 6, "сметнатото не остана видимо"


def test_a_step_of_one_day_is_not_squeezed_to_nothing():
    """Подът от един ден е ненарушим — стъпка, която се прави, не трае нула."""
    from src.segment_scale import _отрежи_края

    задачи = [{"id": "К", "chain_step": "s0", "duration": 1,
               "start_day": 46, "end_day": 46}]
    assert _отрежи_края(задачи, 3) == 0
    assert задачи[0]["duration"] == 1
