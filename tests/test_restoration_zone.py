...
def test_a_zone_asphalts_the_street_once():
    """Всяка стъпка се изпълнява веднъж, щом ѝ принадлежи един ред от КСС.

    Асфалтът е един ред и трябва да е една задача — трите му копия бяха
    дефектът.

    Бордюрите и унипаважът бяха ДВЕ задачи в стъпката за бордюри, защото тя
    ги поглъщаше и двете.  От 19.08.2026 унипаважът е СВОЯ стъпка
    (`sidewalk`): 86% от всички дни за настилки седяха в тази една стъпка,
    с медиана 16 дни, докато в еталона медианата е 2.
    """
    ...
    assert counts["kerbs"] == 1
    assert counts["sidewalk"] == 1
