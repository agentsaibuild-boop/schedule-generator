...
# ---------------------------------------------------------------------------
# Провален прогон също се е платил
# ---------------------------------------------------------------------------

def test_прогон_без_цена_я_смята_от_токените():
    """8 от 40 записани прогона казваха $0.0000, макар да са изгорели токени.

    Сборът излизаше $0.4595 при действителни $0.6333 — занижен с 37 %.
    """
    from tools.rerun_series import _цена_по_токени

    цена = _цена_по_токени({"tokens_in": 12679, "tokens_out": 7})
    assert цена > 0, "провал с 12 679 входни токена не струва нула"


def test_прогон_без_нито_един_токен_струва_нула():
    from tools.rerun_series import _цена_по_токени

    assert _цена_по_токени({"tokens_in": 0, "tokens_out": 0}) == 0.0
    assert _цена_по_токени({}) == 0.0
