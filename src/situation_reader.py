...
def read_water_table(path: str | Path) -> list[Segment]:
    ...
    with doc:
        for page in doc:
            ...
            граници = _roles_of(_header_columns(лента))
            намерени = {р for _, р in граници if р}
            ако_липсва = {r for _, r in _WATER_COLUMNS} - намерени
            if ако_липсва:
                logger.warning("Таблица %s: не са намерени колони %s",
                               име_на_документа, sorted(ако_липсва))
                continue
