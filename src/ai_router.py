...
def worker_max_tokens() -> int:
    ...
    17.08.2026: и 32 000 не стигат.  Сглобяването на пакета за одитора се
    отряза ДВА пъти подред на точно 32 000/32 001 изходни токена — тоест таванът
    пак е водеща причина за провал, само че при по-голям изход.  Проверено
    срещу доставчика: `deepseek/deepseek-v4-flash` обявява **384 000** макс.
    изход, тоест 32 000 е наша настройка, не негово ограничение.

    Тук е 48 000, а не 384 000, и не 64 000.  Таванът се плаща във ВРЕМЕ:
    работникът пише отговора със скорост, при която 64 000 токена траят над
    ПОЛОВИН ЧАС на един опит — измерено същия ден, когато вдигнах тавана и
    сглобяването на пакета увисна на 37 минути с движение по сметката под
    цент.  Тоест едно тясно място се смени с друго: отрязването изчезна, а
    човекът пред екрана зачака.

    48 000 е над най-дългия ЧИСТ отговор, мерен в контролната серия (~43 000),
    и достатъчно под границата, отвъд която един опит става по-дълъг от
    търпението.  Стъпалото надолу в `_chat_deepseek` пази работниците с
    по-нисък таван.
    """
    return int(os.getenv("WORKER_MAX_TOKENS", "48000"))
