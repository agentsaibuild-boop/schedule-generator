...
def worker_max_tokens() -> int:
    ...
    Тук е 64 000, а не 384 000: чистите прогони ползват до ~50 000 (мерено в
    контролната серия), затова 64 000 дава запас, без една засечка да струва
    колкото цял ден генерации.  Стъпалото надолу в `_chat_deepseek` пази
    работниците с по-нисък таван.
    """
    return int(os.getenv("WORKER_MAX_TOKENS", "64000"))
