...
#: СИЛНО доказателство — норма за ТОЗИ диаметър и материал, или число, обявено
#: от изпълнителя за ТОЗИ обект.  Медианата от корпуса е основание, но е
#: по-слабо: тя описва средния участък на друг обект.
#:
#: ЗАЩО ДВЕ МЕТРИКИ.  Одиторът, 31.08.2026: „Технически 0 недоказани следва от
#: новата таксономия, но това не значи, че 100 % от продължителностите са силно
#: доказани.  Бих държал две отделни метрики — това предотвратява бъдещо
#: маркетингово самоизлъгване с „всичко е доказано"."
#:
#: Прав е, и разликата е голяма: Тръстеник е 100 % на доказателство и само 8 %
#: на СИЛНО доказателство.
СИЛНО_ДОКАЗАНИ = frozenset({"PARAMETRIC_NORM", "CONTRACTOR_INPUT"})
...
def grade_of(task: dict) -> str:
    ...
    if ако_е == "step_rate":
        # Темпо на СТЪПКА (`step_productivities` в productivities.json) —
        # изкопът, разкъртването и извозването нямат свой ред в количествата.
        # Числата там носят `източник: „ИЗВЕДЕНА от еталонния човешки график"`,
        # тоест основанието им е същото като на медианите, само че изразено в
        # метри на ден.  Първата версия на стълбицата ги слагаше в UNSUPPORTED
        # и обявяваше 33 задачи без основание, каквито те не са.
        return "HISTORICAL_CORPUS_MEDIAN"
...
def report(tasks: Iterable[dict]) -> dict:
    ...
    силни = sum(к["задачи"] for о, к in по_основание.items()
                if о in СИЛНО_ДОКАЗАНИ)
    return {
        "по_основание": {о: по_основание[о] for о in СТЪЛБИЦА
                         if о in по_основание},
        "задачи": листа,
        "доказани": доказани,
        "силно_доказани": силни,
        "дял_доказани": round(доказани / листа, 4) if листа else 0.0,
        "дял_силно_доказани": round(силни / листа, 4) if листа else 0.0,
...
def describe(отчет: dict) -> list[str]:
    ...
    редове = [f"ОСНОВАНИЕ ЗА ПРОДЪЛЖИТЕЛНОСТИТЕ ({отчет['задачи']} задачи): "
              f"{отчет['дял_доказани']:.0%} на доказателство, от които "
              f"{отчет['дял_силно_доказани']:.0%} на СИЛНО доказателство "
              f"(норма за този диаметър или число от изпълнителя):"]
