...
def fit_contract_span(tasks: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    def _затварящата_точка_на_целта() -> int:
        """Приемането се подписва на договорния ден, не на стара дата.

        Финалният milestone е с нулева продължителност и никой не го притегля
        назад: остане ли от по-дълга разписка, обектът „свършва" на ден 429 при
        строителство, което е приключило на 255.  Тук той се слага на целевия
        ден, стига всичко, което чака, да е готово дотогава.
        """
        по_ид = {str(t.get("id")): t for t in tasks or []}
        преместени = 0
        for t in tasks or []:
            ако_точка = bool(t.get("milestone") or t.get("is_milestone"))
            if not ако_точка or t.get("is_summary"):
                continue
            if int(_num(t.get("start_day"))) <= цел:
                continue
            предшественици = [по_ид.get(_dep_id(d))
                              for d in (t.get("dependencies") or [])]
            най_късен = max((int(_num(п.get("end_day"))) for п in предшественици
                             if п is not None), default=0)
            if най_късен <= цел:
                t["start_day"] = цел
                t["end_day"] = цел
                преместени += 1
    ...
    if недостиг <= 0:
        _надзорът_до_края()
        върнати = _затварящата_точка_на_целта()
        бележка = (f"Договорен срок: {върнати} затварящи точки върнати на ден "
                   f"{цел} — работата преди тях е приключила." if върнати else "")
        return tasks, ([бележка] if бележка else [])
...
def dispatch_sections(
    ...
    #: Кои клонове се раздават динамично.  СЪОРЪЖЕНИЯТА ВЛИЗАТ (26.08.2026):
    #: шахтата също е работа, която един екип кара наведнъж, и е на неговия
    #: фронт.  Останеха ли извън раздаването, те висят там, където по-ранните
    #: проходи са ги оставили — мерено на Тръстеник: два пакета „Етап 1 от 1"
    #: стояха на ден 390, докато екипът им беше свободен от 225, и сами вдигаха
    #: срока на фазата от ~230 на 352 дни.  Настилките и кабелите остават
    #: настрана: те са възстановяване и чужд изпълнител.
    подлежащи = [pid for pid in листа
                 if верига_на.get(pid) in _ЗА_РЕДИЦА_НА_ЕКИПА
    ...
    # Махат се ВСИЧКИ връзки между раздадени клонове от ЕДНА И СЪЩА мрежа.
    #
    # Не само старата редица: `link_cross_discipline` е вързал водопроводните
    # клонове един за друг „на вълни", по СТАРАТА подредба.  Новата редица ги
    # подрежда иначе и двете заедно затварят КРЪГ — мерено: В54 → В106 → … →
    # В54, критичният път излиза 0 и графикът не се подрежда топологично.
    #
    # Редът вътре в мрежата вече е на раздаването; редът МЕЖДУ мрежите (каналът
    # преди водопровода) остава — той е физика, не подредба.
    от_пакет = {str(t.get("id")): pid for pid, деца in листа.items() for t in деца}
    раздадени = set(подлежащи)
    махнати = 0
    for pid in подлежащи:
        for t in листа[pid]:
            ...
            for d in deps:
                ид = _dep_id(d)
                чужд = от_пакет.get(ид)
                своя_мрежа = (чужд in раздадени and чужд != pid
                              and мрежа_на.get(чужд) == мрежа_на.get(pid))
                стара_редица = (isinstance(d, dict)
                                and str(d.get("reason", "")).startswith(
                                    "последователна работа"))
                if своя_мрежа or стара_редица:
                    махнати += 1
                    continue
                остават.append(d)
