...
def order_chronologically(tasks: list[dict]) -> list[dict]:
    ...
    Пренарежда се САМО най-горното ниво.  Стъпките вътре в участък вървят по
    технологичната верига, а тя не винаги е хронологична (изпитване чака
    засипка, която чака друг участък); пипането им би счупило четенето.
    """
    ...
    корени.sort(key=lambda t: (int(t.get("start_day", 1) or 1), ред[id(t)]))
