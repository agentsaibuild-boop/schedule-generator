...
_РАЗАРХИВИРАНО = "разархивирано"


def _име_от_архив(име: str) -> str:
    """Име на файл от ZIP, прекодирано от стария формат за кирилица.

    Архиватори под Windows пишат имената в cp866 и ги обявяват като cp437.
    Без прекодиране „СИТУАЦИЯ.pdf" излиза като „ЫШЛаРдШп.pdf".
    """
    try:
        return име.encode("cp437").decode("cp866")
    except (UnicodeEncodeError, UnicodeDecodeError):
        return име


#: Маркер, че архивът е изваден ДОКРАЙ.  Без него прекъснато вадене минаваше
#: за готово и половината файлове оставаха невидими завинаги.
_ГОТОВО = ".разархивирано-докрай"


def _свободно_име(папка: Path, име: str) -> Path:
    """Път в папката, който не застъпва вече изваден файл.

    Плоското вадене може да срещне две еднакви имена от различни подпапки на
    архива; второто получава наставка, вместо да изяде първото.
    """
    кандидат = папка / име
    if not кандидат.exists():
        return кандидат
    основа, наставка = кандидат.stem, кандидат.suffix
    for n in range(2, 100):
        друг = папка / f"{основа} ({n}){наставка}"
        if not друг.exists():
            return друг
    return кандидат


def _безопасен_път(име: str) -> Path | None:
    """Относителният път на член от архив, или None ако сочи навън.

    Архив може да съдържа „../../нещо" и да пише където си иска (zip slip).
    Тук се пази само това, което остава в целевата папка.
    """
    парчета = [ч for ч in Path(име.replace("\\", "/")).parts
               if ч not in ("", ".", "/")]
    if any(ч == ".." for ч in парчета) or Path(име).is_absolute():
        return None
    return Path(*парчета) if парчета else None
...
class FileManager:
    ...
    def _extract_archives(self) -> list[Path]:
        """Разархивирай ZIP-овете в проекта, за да се виждат файловете в тях.

        ТРЪЖНИЯТ ПАКЕТ ИДВА КАТО АРХИВИ.  Проверено 21.08.2026: чертежите на
        реалния търг — тези, от които идват участъците — стоят в
        `РПИП-2_Илиянци_Канал.zip` и `РПИП-2_Илиянци_Водос.zip`.  Дотук
        приложението изобщо не ги виждаше: `.zip` не е сред поддържаните
        разширения, а никой не разархивира.  Тоест човекът трябваше да отвори
        архивите на ръка, за да работи програмата — при положение, че точно
        така ги дава възложителят.

        Имената вътре често са с кирилица в стар кодов формат (cp437/cp866) —
        затова се прекодират, иначе излизат нечетими.

        Пътищата се ПОЧИСТВАТ: член на архив, който сочи навън („../"), се
        пропуска.  Архив не бива да може да пише където си иска.

        Returns:
            Разархивираните файлове (само поддържаните разширения).
        """
        import zipfile
        ...
        цел_корен = self.base_path / _РАЗАРХИВИРАНО
        извадени: list[Path] = []

        for архив in sorted(self.base_path.rglob("*.zip")):
            if цел_корен in архив.parents:
                continue                      # вече извадено, не се рекурсира
            цел = цел_корен / архив.stem
            маркер = цел / _ГОТОВО
            if маркер.exists():
                continue                      # веднъж разархивиран стига
            от_този: list[Path] = []
            try:
                with zipfile.ZipFile(архив) as z:
                    for info in z.infolist():
                        if info.is_dir():
                            continue
                        име = _име_от_архив(info.filename)
                        безопасно = _безопасен_път(име)
                        if безопасно is None:
                            logger.warning("Пропуснат член на архив извън "
                                           "папката: %s", info.filename)
                            continue
                        if безопасно.suffix.lower() not in SUPPORTED_EXTENSIONS:
                            continue
                        # ПЛОСКО, не по дървото на архива.  Windows къса на 260
                        # знака, а тръжните архиви носят папка със собственото
                        # си име плюс подпапки — реалният път излизаше извън
                        # границата и цял архив не се вадеше (проба 21.08.2026).
                        # За класификацията и без това е важно ИМЕТО, не къде е
                        # било вътре.
                        изход = _свободно_име(цел, безопасно.name)
                        изход.parent.mkdir(parents=True, exist_ok=True)
                        изход.write_bytes(z.read(info))
                        от_този.append(изход)
            except (zipfile.BadZipFile, OSError) as exc:
                logger.warning("Архивът %s не се извади докрай: %s",
                               архив.name, exc)
                continue
            # Папката се създава и когато нищо не е извадено: архив само с
            # .dwg файлове е редовен случай, а без маркер щеше да се пробва
            # наново при всяко изброяване.
            цел.mkdir(parents=True, exist_ok=True)
            маркер.write_text("", encoding="utf-8")
            извадени.extend(от_този)
            logger.info("Разархивиран %s: %d файла", архив.name, len(от_този))
    ...
    # ЧЕРТЕЖИТЕ НЕ СЕ КАЗВАТ „ситуация" — казват се „СИТ".  Проверено върху
    # реалния тръжен пакет (21.08.2026): нито един от файловете, които носят
    # участъците, не се разпознаваше:
    #
    ...
    #
    # Тоест четецът на чертежи работеше, но приложението не му подаваше нищо.
    # „сит" се търси СЛЕПЕНО с разделител („_сит", „сит_", „сит."), а не голо:
    # голото съвпада в „композит", „депозит", „визит".
    _SITUATION_KEYWORDS: frozenset[str] = frozenset({
        "ситуация", "ситуат", "_сит", "сит_", "сит.", "трасе", "трасировъч",
        "situation", "situacia", "_sit", "sit_", "investicia", "инвестиции",
        "site plan", "генерален план", "ген.план", "генплан",
    })
