...
class AIProcessor:
    ...
    def generate_packages(
        ...
        *,
        ...
        # КОЛОНИТЕ, А НЕ САМО РЕДОВЕТЕ.  Проверката отгоре гледа стойности и
        # затова не може да различи „документът мълчи" от „ние не сме го чули":
        # празна мярка е законен отговор.  Дамяново, 31.08.2026 — заглавие
        # „М-КА" вместо „Ед. мярка" — даде 3148 реда без мярка и нула линейни
        # метра, без нито едно предупреждение.  Разликата се вижда само в
        # ЗАГЛАВИЯТА, затова те се одитират отделно.
        try:
            from src.provenance import audit_columns, describe_column_audit
            колони = audit_columns(project_path) if project_path else []
        except Exception:                            # pragma: no cover
            колони = []
        for ред in describe_column_audit(колони):
            _prog(ред)
        ...
        if срок:
            ...
            def _обхвати(колко) -> dict[str, float]:
                ...
                # ПРОБАТА СЕ МЕРИ С ТЕМПОТО, НЕ С НОРМИТЕ (26.08.2026).
                #
                # Продължителностите в готовия график идват от обявения срок
                # (`deadline_pace`), а пробата ги смяташе по нормите — тоест
                # оразмеряваше екипите за друга работа, различна от тази, която
                # после излиза.  Мерено на Илиянци: сметката даваше 1 воден и
                # 2 канализационни екипа, а обектът иска 3 и 6, и срокът
                # излизаше 1160 дни вместо 780.
                #
                # Тук темпото е ПОДЪТ — най-бързото реално — защото пробата
                # търси МИНИМАЛНИЯ брой екипи.  Ако той не стигне, самата проба
                # го вдига по-нататък.
                from src.deadline_pace import _ПОД_НА_ТЕМПОТО

                зад, _ = calibrate_to_declared_pace(
                    зад, проба, boq_index, overrides=dict(_ПОД_НА_ТЕМПОТО))
            ...
            # ОПИТ (26.08.2026): екипите направо от метрите и пода, без проба.
            # Не е правило — стои зад флаг, за да се мери едно до друго.
            if os.getenv("CREWS_FROM_METRES", "0") not in ("0", "false", ""):
                from src.crew_sizing import crews_from_metres

                минимум, бележки = crews_from_metres(packages, срок)
            else:
                минимум, бележки = crews_for_deadline(пробни, проба, срок)
            for бележка in бележки:
                _prog(бележка)
            # СМЕТКАТА ОТ МЕТРИТЕ НЕ СЕ ДОЗИРА.  Дозирането разписва пробата и
            # вдига екипите, докато тя се събере — но пробата не съдържа нито
            # раздаването, нито реда между мрежите, затова иска повече, отколкото
            # трябва.  Мерено на Тръстеник: сметката дава 10 водни и 5
            # канализационни, дозирането ги вдига на 12 и 12, а подът на темпото
            # веднага обявява, че 12 са в повече и стигат 10 и 5.  Двете правила
            # не бива да си противоречат в един и същи график.
            от_метрите = os.getenv("CREWS_FROM_METRES", "0") not in ("0", "false", "")
            if минимум and от_метрите:
                екипи = dict(минимум)
                _prog("Екипите идват от сметката с метрите и не се дозират "
                      "допълнително — пробата не съдържа раздаването на "
                      "клоновете и иска повече, отколкото обектът изисква.")
            elif минимум:
        ...
        # ТЕМПОТО СЕ ЗНАЕ ПРЕДИ РАЗДАВАНЕТО НА ЕКИПИ (26.08.2026).  Без него
        # фронтовете се балансираха по МЕТРИ, а екипът се заема с ДНИ: клон от
        # 30 м иска девет стъпки по поне ден, също като клон от 300 м.  Мерено
        # на Тръстеник — осем екипа с равни метри излязоха с 81 до 276 дни
        # работа, а срокът се определя от най-натоварения.
        from src.deadline_pace import derive as _темпо_от_срока

        темпа_за_баланс, _ = _темпо_от_срока(
            packages, days=срок, crews=екипи,
            parallel=teams_work_in_parallel())
        packages = assign_fronts(packages, екипи, темпа=темпа_за_баланс,
                                 chains=chains)
        ...
        # ЗАКЛЮЧИТЕЛНО СВЕРЯВАНЕ С ГРАФА (25.08.2026).  Изнесеният график
        # трябва да е съгласуван със СОБСТВЕНИТЕ СИ зависимости — иначе
        # човекът чете едно, а връзките казват друго.
        #
        # Мерено на Русе: договорният milestone „Приемане на обекта (Акт
        # 15/16)" стоеше на ден 313, докато всичко останало свършва на 148, а
        # чисто преразписване го слага на 149.  Причината: преразписването
        # става САМО когато някое правило е сменило продължителност, а на
        # последната итерация нищо не се мени — и задачата оставаше на стара
        # дата от по-дълга разписка.  Едно число, но то дърпаше отчетения срок
        # със 165 дни и вдигаше блокер за превишен договорен срок.
        tasks = _преразпиши(tasks)
        tasks, _ = enforce_construction_span(tasks)
        ...
        #
        # Изключва се с `DISPATCH_SECTIONS=0` — за сравнение при мерене.
        # Работи заедно с пода на темпото (`deadline_pace._ПОД_НА_ТЕМПОТО`):
        # без него обект с малко метри и много екипи получаваше нереално бавно
        # темпо и срокът избухваше.
        if os.getenv("DISPATCH_SECTIONS", "1") not in ("0", "false", ""):
            tasks, dispatch_notes = dispatch_sections(tasks, packages)
            for note in dispatch_notes:
                _prog(note)
            # СЛЕД РАЗДАВАНЕТО ГРАФИКЪТ СЕ ПРЕРАЗПИСВА.  Раздаването мести
            # клоновете и пренаписва връзките в редицата; всичко, което зависи
            # от тях — приемането, договорните точки — трябва да ги последва.
            # Без това приемането оставаше на стара дата: ден 428 при
            # строителство, което свършва на 255.
            tasks = _преразпиши(tasks)
            tasks, _ = enforce_construction_span(tasks)
        ...
        #
        # Дотук CPM вървеше ПРЕДИ `fit_contract_span` и `sync_durations_to_span`,
        # тоест резервите описваха график, който след това се променяше.
        # Личеше си по невъзможно число (Тръстеник, 31.08.2026): задача, която
        # свършва на ден 255 при проект до ден 255, носеше резерв 103 дни, а
        # критични бяха 1 от 895 листа — тоест изнесеният за MS Project график
        # нямаше критична верига и не ставаше за анализ на закъснения.
        cpm = builder.compute_critical_path(tasks)
        if not cpm["warnings"]:
            tasks = cpm["schedule"]
        else:
            # По-добре без критичен път, отколкото с грешен — но се КАЗВА.
