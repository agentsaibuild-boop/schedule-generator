...
def _отпечатък_на_лист(rows: list) -> tuple:
    """Кои количества носи листът — за да се познае повторение.

    ЗАЩО.  Дамяново, 31.08.2026: работният файл има ДВА листа с едни и същи
    позиции — „КС" (без цени) и „КСС" (с цени), по 1244 количества всеки.
    Индексът ги четеше и двата и обектът излизаше с 21 611 м тръби вместо
    10 806.  Двойно количество значи двойна работа и двоен срок — тихо.

    Отпечатъкът е по СТОЙНОСТИТЕ (описание, количество, мярка), не по името на
    листа: същите числа под друго заглавие пак са същите числа.
    """
    записи = []
    for ред in rows or []:
        if not isinstance(ред, dict):
            continue
        кол = _number(_pick(ред, _QTY_KEYS, prefer_numeric=True)[1])
        if кол is None:
            continue
        описание = str(_pick(ред, _DESC_KEYS)[1] or "").strip().lower()
        записи.append((описание[:80], round(float(кол), 4),
                       str(_pick_unit(ред) or "").strip().lower()))
    return tuple(записи)
...
def build_quantity_index(base_path: str | Path) -> list[QuantityRow]:
    ...
    for jf in sorted(converted.glob("*.json")):
        ...
        видени_отпечатъци: dict[tuple, str] = {}
        for sheet in data.get("sheets") or []:
            sheet_name = sheet.get("name", "")
            отпечатък = _отпечатък_на_лист(sheet.get("rows") or [])
            if отпечатък and отпечатък in видени_отпечатъци:
                logger.warning(
                    "Лист „%s“ в %s носи СЪЩИТЕ %d количества като „%s“ — не "
                    "се индексира втори път.  Обичайно е „КС“ (без цени) и "
                    "„КСС“ (с цени) да са един и същи опис.",
                    sheet_name, document, len(отпечатък),
                    видени_отпечатъци[отпечатък])
                continue
            if отпечатък:
                видени_отпечатъци[отпечатък] = sheet_name
