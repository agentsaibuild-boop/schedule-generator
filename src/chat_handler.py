...
class ChatHandler:
    ...
    def _handle_sequence_answer(self, user_message: str, state: dict) -> dict:
        ...
        if step == "q_laying":
            ...
            return self._ask_single_route(new_state, метод_етикет)

        # ── Q1г: едно трасе или разпределителна мрежа? ──────────────────
        #
        # РЕШАВА КОЛКО ПЪТИ СЕ ПРАВИ ЕДНОКРАТНАТА РАБОТА (25.08.2026).  При
        # непрекъснат водопровод изпитването, дезинфекцията и присъединяването
        # са по ЕДНА задача за целия обект — така е в човешкия график на
        # тласкателя.  При разпределителна мрежа всеки клон се изпитва сам.
        # Разликата не се чете от количествата (и двете са „метри тръба"),
        # затова се пита.  Виж `single_route.collapse_route_wide_steps`.
        if step == "q_route":
            метод_етикет = state.get("_laying_label", "")
            if msg.startswith("Т") or "ТРАСЕ" in msg or "ТЛАСКАТЕЛ" in msg                     or "ДОВЕЖДАЩ" in msg:
                трасе, трасе_етикет = "В", "едно непрекъснато трасе"
            elif msg.startswith("М") or "МРЕЖА" in msg or "РАЗПРЕДЕЛ" in msg:
                трасе, трасе_етикет = "", "разпределителна мрежа"
            else:
                return {**_base, "response": (
                    "Моля, отговори с **Т** (едно трасе) или **М** "
                    "(разпределителна мрежа)."
                ), "pending_sequence": state}

            tender = {**(state.get("tender") or {}), "single_route": трасе}
            return self._ask_contract_days(
                {**state, "tender": tender, "_route_label": трасе_етикет},
                f"{метод_етикет}, {трасе_етикет}")
    ...
    def _ask_single_route(self, state: dict, метод_етикет: str) -> dict:
        """Питай дали обектът е ЕДНО трасе, или разпределителна мрежа.

        Мерилото е човешкият график на тласкателя „Образцов чифлик" (ВиК Русе):
        три изпълнителски участъка, но изпитването, дезинфекцията и
        присъединяването са по веднъж за целия водопровод.  Ние ги повтаряхме
        на всеки участък — осем етапа × три стъпки вместо три задачи.
        """
        _base = {
            "schedule_updated": False,
            "schedule_data": None,
        ...
        return {**_base,
            "response": (
                ...
                "**Обектът едно непрекъснато трасе ли е?**\n"
                "  **Т** — едно трасе (тласкател, довеждащ водопровод)\n"
                "  **М** — разпределителна мрежа с клонове\n\n"
                "При едно трасе изпитването, дезинфекцията и присъединяването "
                "се правят ВЕДНЪЖ за целия водопровод; при мрежа всеки клон "
                "се изпитва и дезинфекцира сам."
            ),
            "pending_sequence": {**state, "step": "q_route"},
        }

    def _брифинг(self, *, за: str = "") -> str:
        """Каквото прочетохме от търга — показва се ПРЕДИ да питаме човека.

        Изпълнителят, 31.08.2026: „След като прочетеш всички документи на
        търга, питай човека зададените въпроси — тук предоставяш информацията,
        която си взел от документацията."

        Дотук питахме на сляпо и карахме човека да отвори документ, който вече
        сме прочели.  При Карлово това значеше да не му покажем, че срокът има
        и ПОД (365 дни) и че излизане извън границите ДИСКВАЛИФИЦИРА.
        """
        път = getattr(self.files, "base_path", None) if self.files else None
        if not път:
            return ""
        try:
            from src.provenance import build_quantity_index
            from src.tender_brief import brief, crew_scenarios, describe

            данни = brief(път, build_quantity_index(път))
            редове = describe(данни)
            if за == "екипи":
                редове += crew_scenarios(данни)
        except Exception as exc:                     # noqa: BLE001
            logger.warning("Брифингът от търга се провали: %s", exc)
            return ""
        if not редове:
            return ""
        return chr(10).join(редове) + chr(10) * 2
    ...
    def _ask_contract_days(self, state: dict, метод_етикет: str) -> dict:
        ...
        return {**_base,
            "response": (
                ...
                "Ако горе съм го извадил вярно — потвърди го.  Ако не съм "
                "извадил нищо или числото е грешно, кажи правилното: то е "
                "максимумът и на графика.\n\n"
                "  напиши **число** (напр. **500**)\n"
    ...
    def _ask_parallel_teams(self, state: dict) -> dict:
        ...
        return {**_base,
            "response": (
                self._брифинг(за="екипи")
                + "**Колко екипи ще работят?**\n\n"
                "Броят екипи е ТВОЙ — организацията е на изпълнителя.  Горе е "
                "какво излиза при всеки брой, за да се избира с числа.\n\n"
                "  **1** — един екип\n"
                "  **2** — два екипа\n"
