...
_MATERIAL_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    ("PE", re.compile(
        # „RC" (Resistant to Crack) върви СЛЕПЕНО след класа: „PE100RC" —
        # стандартното означение за безизкопно полагане, тоест за цял клас
        # обекти (Елхово, 09.2026: и 29-те участъка са сондажни).  Без
        # незадължителния суфикс границата на думата след „100" не се
        # намира и ГРЪБНАКЪТ на участъка остава MISSING_MATERIAL.
        r"\bPEHD\b|\bHDPE\b|\bPE\s*-?\s*(?:HD|100|80)(?:\s*-?\s*RC)?\b"
