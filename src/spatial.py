...
def find_crew_collisions(
    ...
    for i, a in enumerate(items):
        for b in items[i + 1:]:
            # ЕДИН И СЪЩ екип на едни и същи метри НЕ е сблъсък на бригади.
            #
            # Намерението беше записано още в първата версия, но кодът стоеше
            # като `pass` — тоест пропадаше надолу и пак съобщаваше конфликт.
            # ЖИВ ПРОГОН 2026-08-07: технологичната верига разделя една операция
            # на по една задача за всеки ред от КСС (полагане на тръби + бетонов
            # кожух + СКО).  Те са ЕДНА бригада, на едно място, в едни дни — и
            # даваха 35 „конфликта", които обявяваха валидния график за невалиден.
            #
            # Претоварването на един екип е РЕСУРСЕН въпрос и се проверява от
            # `validate_schedule` (застъпване на екипи), не тук.
