...
def _run_conversion(force: bool = False) -> None:
    ...
    if not classification["can_proceed"]:
        status_area.caption(
            "По имена няма таблица с количества — проверявам съдържанието на файловете…"
        )
    ...
    if not classification["can_proceed"]:
        ...
        # КСС НЕ Е задължителна (21.08.2026, проверено с прогон): шест реда
        # „мрежа, дължина, диаметър, материал“ дават чист график.  Затова
        # съобщението казва КОЛКО МАЛКО трябва, вместо да иска цяла сметка.
        warning_lines.append(
            "\n**Какво е достатъчно.** Не е нужна количествено-стойностна сметка. "
            "Стига таблица с колони „мярка“ и „количество“, в която за всеки ред "
            "пише мрежа, дължина, диаметър и материал:"
        )
        warning_lines.append(
            "\n| Описание | Ед. мярка | Количество |\n"
            "|---|---|---|\n"
            "| Полагане на водопровод Ф110, PE | m | 1800 |\n"
            "| Полагане на канализация Ф400, PP | m | 800 |\n"
            "| СКО — сградни канализационни отклонения | брой | 140 |\n"
            "| Възстановяване на асфалтова настилка | кв. м | 9000 |"
        )
        warning_lines.append(
            "\nБройките и площите не са задължителни — само дължините стигат за "
            "график. Ако количествата са в друг документ, уверете се, че таблицата "
            f"е извлечена: сканиран PDF без OCR не се чете{ai_label}."
        )
        st.session_state.messages.append({
            "role": "assistant",
            "content": "\n".join(warning_lines),
    ...
    if classification.get("boq_by_content"):
        ...
        st.session_state.messages.append({
            ...
            "content": (
                ...
                "и количества — а само това е нужно. Продължавам с него."
            ),
        })
...
# да се провери, че таблицата, статистиката и експортът изобщо се рисуват.
#
# ЗАМЕНЯ възстановения график, а не го допълва (17.08.2026).  Досега условието
# беше „само ако няма график", но приложението възстановява предишната сесия
# при старт — тоест стойката се задействаше или не в зависимост от това какво
# е правил човекът последно на тази машина.  Тестовете описваха ту примерния
# график, ту нечий стар, и падаха ту тук, ту там.  Стойка, която зависи от
# историята, не е стойка.
if os.getenv("UI_SMOKE_SCHEDULE") == "1":
    schedule = [
    ...
    # Експортът е ЗАКЛЮЧЕН за график, невалидиран в тази сесия (виж гейта в
    # раздел „Експорт").  Това е правилно и не се заобикаля тук — но без
    # валидация стойката проверява само че бутоните са СИВИ, а дали PDF/XML
    # изобщо се раждат остава непроверено.  Затова примерният график носи и
    # своята валидация, вързана за собствения си hash: щом графикът се смени,
    # тя става невалидна по същия начин, както при истински график.
    st.session_state.last_validation = {
        "checked": True, "valid": True, "errors": [],
        "schedule_hash": AIProcessor.schedule_hash(schedule),
    }
